...
class Line(BaseParser):
    """Line method of parsing looks for lines between text
    to parse the table.
    ...
    def _generate_table(self, table_idx, cols, rows, **kwargs):
        ...
        if self.copy_text is not None:
            table = Line._copy_spanning_text(table, copy_text=self.copy_text)
    ...
    def extract_tables(self, filename, suppress_stdout=False, layout_kwargs={}):
        ...
        if not suppress_stdout:
            print("Processing {}".format(os.path.basename(self.rootname)))

        if not self.horizontal_text:
            if self.images:
                warnings.warn(
                    "{} is image-based, rocket only works on"
