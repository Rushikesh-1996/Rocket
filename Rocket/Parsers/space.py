...
class Space(BaseParser):
    """Space method of parsing looks for spaces between text
    to parse the table.
    ...
    def _add_columns(cols, text, row_tol):
        ...
        """
        if text:
            text = Space._group_rows(text, row_tol=row_tol)
            elements = [len(r) for r in text]
            new_cols = [
                (t.x0, t.x1) for r in text if len(r) == max(elements) for t in r
            ]
            cols.extend(Space._merge_columns(sorted(new_cols)))
        return cols
    ...
    def extract_tables(self, filename, suppress_stdout=False, layout_kwargs={}):
        ...
        if not self.horizontal_text:
            if self.images:
                warnings.warn(
                    f"{base_filename} is image-based, rocket only works on"
                    " text-based pages."
                )
            else:
